									...
									unsigned int sg_len)
{
	...
	else
	{
		...
		for (i = 0; i < sg_len; i++)
		{
			...
			for (; length; desc++)
			{ /*lint !e440*/
				desc_len = (length <= DW_MCI_DESC_DATA_LENGTH) ? length : DW_MCI_DESC_DATA_LENGTH;
...
static void dw_mci_setup_bus(struct dw_mci_slot *slot, bool force_clkinit)
{
	...
	retry:
		...
		if (!(mci_readl(host, INTMASK) & SDMMC_INT_SDIO(slot->id)) && (host->hw_mmc_id != DWMMC_SD_ID) && mmc_host_wifi_support_lowpwr(slot->mmc))
...
int dw_mci_get_cd(struct mmc_host *mmc)
{
	...
	if (brd->quirks & DW_MCI_QUIRK_BROKEN_CARD_DETECTION)
		...
		present = (mci_readl(slot->host, CDETECT) & (1 << id)) == 0 ? 1 : 0;
	if (present)
...
static void dw_mci_disable_low_power(struct dw_mci_slot *slot)
{
	...
	if (clk_en_a & clken_low_pwr)
	{
		...
		(void)mci_send_cmd(slot, SDMMC_CMD_UPD_CLK | SDMMC_CMD_PRV_DAT_WAIT, 0);
	}
}
...
static struct dw_mci_board *dw_mci_parse_dt(struct dw_mci *host)
{
	...
	if (of_find_property(np, "caps2-wifi-support-cmd11", NULL) && of_property_count_u8_elems(np, "caps2-wifi-support-cmd11") <= 0)
	{
	...
	if (of_find_property(np, "caps-wifi-no-lowpwr", NULL) && of_property_count_u8_elems(np, "caps-wifi-no-lowpwr") <= 0)
	{
...
#endif
	mci_writel(host, INTMASK, SDMMC_INT_CMD_DONE | SDMMC_INT_DATA_OVER | SDMMC_INT_TXDR | SDMMC_INT_RXDR | DW_MCI_ERROR_FLAGS);
